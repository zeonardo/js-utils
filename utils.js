...
export function addClass(el, className) {
  ...
  for (var i = classes.length - 1; i >= 0; i--) {
    ...
    else if (!hasClass(el, classes[i])) el.className += ' ' + classes[i];
  }
...
export function formatMoney(value, includeCurrency, digits, separatorDec, separatorThou){
  value = value || 0;
  ...
  let s = value < 0 ? '-' : '',
      ...
      c = includeCurrency ? ((includeCurrency == true ? config.msg.currencySymbol : includeCurrency) + ' '): '';
  return c + s + (j ? i.substr(0, j) + separatorThou : '') + i.substr(j).replace(/(\d{3})(?=\d)/g, '$1' + separatorThou) + (digits ? separatorDec + Math.abs(value - i).toFixed(digits).slice(2) : '');
}
...
export function isScreenSmaller(){
  return getScreenWidth() < config.screenWidthBreakpoint_sm;
}

export function isScreenSmall(orSmaller = false){
  const sw = getScreenWidth()
  return (orSmaller || sw >= config.screenWidthBreakpoint_sm) && sw < config.screenWidthBreakpoint_md;
}

export function isScreenMedium(orSmaller = false){
	const sw = getScreenWidth()
  return (orSmaller || sw >= config.screenWidthBreakpoint_md) && sw < config.screenWidthBreakpoint_lg;
}

export function isScreenLarge(orSmaller = false){
	const sw = getScreenWidth()
  return (orSmaller || sw >=  config.screenWidthBreakpoint_lg) && sw < config.screenWidthBreakpoint_xl;
}

export function isScreenLarger(){
  return getScreenWidth() >= config.screenWidthBreakpoint_xl;
}


export function isTouchable(){
  var nie = window && document && ((('ontouchstart' in window) || window.DocumentTouch && document instanceof DocumentTouch) || false);
  var ie = navigator && (navigator.maxTouchPoints || navigator.msMaxTouchPoints || 0) > 0;
  return nie || ie;
}
...
export function smartHover(elem, inFn, outFn, useCapture){
  let timeout;
  if((!inFn || inFn.constructor !== Function) && (!outFn || outFn.constructor !== Function)){
    return;
  }
  inFn && addListener(elem, 'mouseenter', function(e){
    if(!isScreenMedium(true)){
      clearTimeout(timeout);
      return inFn(e);
    }
  }, useCapture);
  outFn && addListener(elem, 'mouseleave', function(e){
    if(!isScreenMedium(true)){
...
export function smartClick(elem, fn, useCapture){
  ...
  var clicked = false;
  if(isTouchable()){
    addListener(elem, 'touchstart', function(e){
      clicked = new Date().getTime();
    }, useCapture);
    addListener(elem, 'touchmove', function(e){
      clicked = false;
    }, useCapture);
    addListener(elem, 'touchend', function(e){
...
export function kebabCase(name) {
    name = removeAccents(name).replace(/&/g, 'e').replace(/[^._a-zA-Z0-9]/g, '-').replace(/-{2,}/g, '-');
    return name;
}

//TODO: get product link name or full link from API
export function getProductUrl(id, name) {

  if (!id || !name) {
    return '';
  }
  return '/produto/' + id + '/' + kebabCase(name);
}

export function addEvent(element, name, detail, bubbles = false, cancelable = false) {
  ...
  else if('createEvent' in document){//IE
...
export function redirectEvent(element, event, source = 'header', type = 'click', redirectTo){
  if(!element || (!element.getAttribute('href') && !redirectTo) || element.getAttribute('href') == '#'){
    event && ('preventDefault' in event) && event.preventDefault();
    return false;
  }

  return addEvent(element, 'location:redirect', {
    type: type,
...
export function isElementInViewport(element, fully = false) {
  ...
  return (
    !rect || (
			rect.top + (fully ? rect.height : 0) <= (window.innerHeight || document.documentElement.clientHeight) &&
			rect.left + (fully ? rect.width : 0) >= 0 &&
			rect.bottom - (fully ? rect.height : 0) >= 0 &&
			rect.right - (fully ? rect.width : 0) <= (window.innerWidth || document.documentElement.clientWidth)
    )
  );
}
...
export function isQlasTsal() {
	return cookieGet('b2wChannel') && (cookieGet('b2wChannel').toLowerCase() == 'qlas' || cookieGet('b2wChannel').toLowerCase() == 'tsal');
}
...
export default ({ hasClass, removeClass, addClass, addEvent, addListener, formatMoney, kebabCase, isScreenSmall, isScreenMedium, isScreenLarge, isScreenLarger, smartHover, smartClick, isTouchable, debounce, getUrlParameter, redirectEvent, scrollTo, smartKeydown, isQlasTsal, deviceType });
